function [u] = G7_Poisson_Equation_Axb(f, mask_dst, param)
...
mask_dst_ext =zeros(ni+2, nj+2);
mask_dst_ext (2:end-1, 2:end-1) = mask_dst;
...
for j=2:nj+1
    for i=2:ni+1
        ...
        if (mask_dst_ext(i,j)==1) %If we have to inpaint this pixel
    ...
    %TO COMPLETE 7
    A_size = (ni+2)*(nj+2);
    A=sparse(idx_Ai, idx_Aj, a_ij, A_size, A_size); %??? and ???? is the size of matrix A
