...
while dif>tol && nIter<iterMax
    ...
    c1 = sum(I .* ((I-phi)>=0),'all') / (sum((I-phi)>=0,'all')); %TODO 1: Line to complete
    c2 = sum(I .* ((I-phi)< 0),'all') / (sum((I-phi)< 0,'all')); %TODO 2: Line to complete
    ...
    subplot(2,2,[1,3]) 
        ...
        s = surfc(phi);  %TODO 16: Line to complete 
        set(s,'LineStyle','none');
        hold on
        %The zero level set over the surface
        contour(phi, 'red', 'LevelList', [0], 'LineWidth', 3); %TODO 17: Line to complete
        hold off
        title('Phi Function');
